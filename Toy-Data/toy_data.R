...
## load packages for data generation
library(survival)
library(tidyverse)


#' simulate_weibull_data
#'
#' @param n        sample size
#' @param beta     regression coefficients on covariates
#' @param shape    shape parameter of Weibull distribution
#' @param rate_C   rate parameter for censoring variable C
#'
#' @returns dataframe containing covariates necessary to fit each of the four models
#' @export
#'
#' @examples
simulate_weibull_data <- function(n, beta, shape, rate_C) {
  # Generate covariates used in models
  Age <- rnorm(n)                                              # Age
